...
public class TestRunArguments {
	...
	/**
	 * Validates the given test run arguments. An <code>IllegalArgumentException</code> is
	 * thrown if a required argument is missing or invalid. The recognized arguments are
	 * listed below.
	 * <ul>
	 * <li>{@value org.opengis.cite.wcs11.WCS11#URL} (required)</li>
	 * </ul>
	...
	public static Properties validateArguments(Document testRunArgs) {
		...
		String capabilitiesUrl = args.getProperty(WCS11.URL);
		try {
			...
			throw new IllegalArgumentException(String.format("Missing required argument: %s", WCS11.URL));
		}
		return args;
	}
